...
export async function GET(request: NextRequest, { params }: { params: Promise<{ noradId: string }> }) {
  try {
    ...
    const localSatellite = CANADIAN_SATELLITES.find((s) => s.noradId === noradIdNum)
    if (localSatellite) {
      const realisticSatellite = generateRealisticTLE(localSatellite)
      return NextResponse.json(realisticSatellite)
    }
